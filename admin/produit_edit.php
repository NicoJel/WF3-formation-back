...
$nom = $description = $reference = $prix = $categorie = '';
...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES['photo']['tmp_name'])) {
        ...
        if (!in_array($_FILES['photo']['type'], $allowedMimeTypes)) {
            $errors[] = 'La photo doit être une image GIF, JPG, PNG';
        }
    }
    if (empty($errors)) {
        ...
        if (!empty($_FILES['photo']['tmp_name'])){
            ...
            // le nom que va avoir le fichier dans le repertoire photo
            $nomPhoto = $_POST['reference'] . $extension;
            
            // en modification, si lme produit avait déjà une photo, on la supprime
